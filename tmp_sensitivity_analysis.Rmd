...
tmp_forcing_data %>% 
  ...
  geom_line(color = "#35978F") + facet_wrap(~name, scales = "free") +
...
bind_rows(runlist) %>% 
  ...
  facet_wrap(~parameter, scales = "free", 
             labeller = as_labeller(c(param_bulkd = "Bulk Density (kg/m3)", param_claysilt = "%claysilt (%)",
                                      param_pc = "pc (?)", param_pH = "pH (unitless)"))) + 
  theme_minimal(base_size = 24)
...
ribbon_plot_data %>% 
  filter(metric == "CO2") -> co2

deltaCO2 <- tibble(time = co2$time,
                   metric = "CO2_emissions",
                   sd = c(NA, diff(co2$sd)), 
                   min = c(NA, diff(co2$min)),
                   max = c(NA, diff(co2$max)),
                   median = c(NA, diff(co2$median)))

# plot co2 emissions
ggplot(deltaCO2, aes(x = time)) +
  geom_ribbon(aes(ymin = min, ymax = max), fill = "#1E3C6E", alpha = 0.5) +
  geom_line(aes(y = median), color = "#1E3C6E") +
  labs(x = "Time (days)", y = expression(paste("CO2 Emissions (gC ", m^{-2}, d^{-1}, ")"))) +
  theme_minimal(base_size = 24)
...
ribbon_plot_data %>%
  ...
  labs(x = "Time (days)", y = expression(paste("gC ", m^{-2}))) +
  theme_minimal()
